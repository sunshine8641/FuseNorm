...
try:
    from .activation import get_activation, requires_num_channels
except ImportError:
    from activation import get_activation, requires_num_channels
...
class WideBasic(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, dropout_rate=0.0, activation="relu"):
        ...
        act1 = get_activation(activation, num_channels=in_planes if requires_num_channels(activation) else None)
        self.act1 = act1
        self.conv1 = conv3x3(in_planes, out_planes, stride)
        self.dropout_rate = dropout_rate
        self.dropout = nn.Dropout(p=dropout_rate) if dropout_rate > 0 else nn.Identity()
        self.bn2 = nn.BatchNorm2d(out_planes)
        act2 = get_activation(activation, num_channels=out_planes if requires_num_channels(activation) else None)
        self.act2 = act2
...
class WideResNet(nn.Module):
    ...
    def __init__(self, depth=28, widen_factor=10, num_classes=1000, dropout_rate=0.0, cifar=True, activation="relu",last_block_activation=None):
        ...
        if cifar:
            ...
            self.act = get_activation(activation, num_channels=16 if requires_num_channels(activation) else None)
            ...
            self.act = get_activation(activation, num_channels=64 if requires_num_channels(activation) else None)
            self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
