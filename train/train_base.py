...
def run_one_epoch_base(model,optimizer, dataloader, criterion, accelerator, top_k, meters, mode="Train"):
    ...
    with torch.no_grad() if mode != "Train" else contextlib.nullcontext():
        for inputs, targets in dataloader:
            ...
            with accelerator.autocast():  # 自动混合精度
                outputs = model(inputs)
                loss = criterion(outputs, targets)
            ...
            #     if isinstance(m, ParametricBReLU):
            #         reg_loss += 1e-5 * torch.sum(m.c ** 2)
            #         reg_loss += 1e-5 * torch.sum(m.l ** 2)
            # loss += reg_loss
            # --------------------
